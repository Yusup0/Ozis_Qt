...
//    if (!index.isValid() || data_pos_.isEmpty() || index.row() > data_pos_.size() || index.column() > 7)
//        return QVariant();
...
void ProductModel::ReadFile() {
    if (!file_->open(QFile::ReadOnly)) return;
    addresses_.clear();
    ...
    while (!file_->atEnd()) {
        ...
        if (addresses_.back().category_.isEmpty() || addresses_.back().category_.back().id_ != category_id) {
            addresses_.back().category_.push_back(Contract::Category(category_id, pos));
        } else {
...
void ProductModel::SetAdress() {
    ...
    if (contract_id_ != 0 && category_id_ != 0) {
        for(auto &i: addresses_) {
            if (i.id_ != contract_id_) continue;
            for(auto j: i.category_) {
                if (j.id_ != category_id_) continue;
...
void ProductModel::SetId(int contract_id, int category_id) {
    ...
    count = 0;
    count1 = 0;
    if (!data_pos_.isEmpty()) file_->seek(data_pos_[0].first);
}
...
void ProductModel::SetFile(QFile *file) {
    file_ = file;
}
