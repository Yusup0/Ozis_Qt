...
void MainWindow::on_pushButton_5_clicked() {
  ...
  ui->comboBox_2->addItems(
      GetListBD(QFile(beginning_path_ + "/bd/contract.csv")));
  ui->comboBox_3->addItems(
      GetListBD(QFile(beginning_path_ + "/bd/category.csv")));
  data_.clear();
  animation_right->start();
}
...
void MainWindow::ResponseBearer() {
  int status_code =
      api_.reply_->attribute(QNetworkRequest::HttpStatusCodeAttribute)
          .value<int>();
  if (status_code == 200) {
...
void MainWindow::ResponseBDAdd() {
  ...
  if (name == "contract") {
    ...
    list << ui->lineEdit_8 << ui->lineEdit_11 << ui->lineEdit_12
         << ui->lineEdit_13;
...
void MainWindow::PriceChange(int number) {
  ...
  ui->label_15->setText(QString("<p align=center><font size=5>%L1</font>")
                            .arg(order_.GetTotalPrice()));

  QString ob_name = "label_" + sender()->objectName().section('_', 1, 1);
  double price =
      order_.QuantityInOrder(ob_name.section('_', 1, 1).toInt()).second;

  for (auto &widget : ui->scrollArea_3->widget()->children()) {
    if (widget->objectName() == ob_name) {
      static_cast<QLabel *>(widget)->setText(
          QString("%L1").arg(number ? (QString::number(price * number)) : ""));
      break;
    }
  }
}
...
void MainWindow::on_pushButton_2_clicked() {
  ...
  qDebug() << "загрузка";
  connect(api_.reply_, &QNetworkReply::finished, this,
          &MainWindow::ResponseArchiveBD);
}
...
void MainWindow::OrderOnWidgit(QScrollArea *scroll_area,
                               QList<QList<QString>> date) {
  ...
  while (date_it != date.end()) {
    ...
    QDir dir(*(data_dir));
    auto list_file = dir.entryList(
        QDir::AllEntries | QDir::Files | QDir::NoDotAndDotDot, QDir::Time);
    ...
    while (it != list_file.end()) {
      ...
      if (file.open(QIODevice::ReadOnly)) {
        ...
        layout_h->GetPhoto()->setPixmap(
            QPixmap(QString(beginning_path_ + "/bd/photo_store/%1/logo.jpg")
                        .arg(*line_it)));
        layout_h->GetLabel()->setObjectName(dir.path() + "/" + number_order);
        QString name =
            GetNameBD(beginning_path_ + "/bd/retailer.csv", *line_it);
        layout_h->GetLabel()->setText(
            QString("<p> <span  align=left><font size=5>№%1 %2</font><br/>"
                    "<br/><font size=3>%3, сумма - %4 руб.</font></p>")
                .arg(number_order, name, *(++line_it), *(++line_it)));
        connect(layout_h->GetLabel(), &MyLabel::clicked, this,
                &MainWindow::Label4Clicked);
...
void MainWindow::RetailerOnWidgit(QScrollArea *scroll_area) {
  ...
  if (file.open(QIODevice::ReadOnly)) {
    while (!file.atEnd()) {
      ...
      layout_h->GetPhoto()->setPixmap(QPixmap(
          QString(beginning_path_ + "/bd/photo_store/%1/logo.jpg").arg(*it)));
      layout_h->GetLabel()->setText(
          QString("<p align=left><font size=5>%1</font><br/><br/><font "
                  "size=3>%2</font></p>")
              .arg(*(++it), *(++(++it))));
      connect(layout_h->GetLabel(), &MyLabel::clicked, this,
              &MainWindow::LabelClicked);
      layoutV->addLayout(layout_h);
    }
    file.close();
  }
...
void MainWindow::ReadSettings() {
  ...
  user_.SetPassword(
      settings_.value("/password", "there is no password").toString());
  settings_.endGroup();
}
...
void MainWindow::ContractOnWidgit(QScrollArea *scroll_area) {
  ...
  for (auto &contract : contracts) {
    ...
    lable->setText(
        QString("<p align=left><font size=5>%1").arg(contract.second));
...
void MainWindow::CategoryOnWidgit(QScrollArea *scroll_area) {
  ...
  for (auto &category : categories) {
    ...
    lable->setText(
        QString("<p align=left><font size=5>%1").arg(category.second));
...
void MainWindow::ProductOnWidgit(QScrollArea *scroll_area) {
  ...
  if (file_product_->open(QFile::ReadOnly)) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      layout_h->GetPhoto()->setPixmap(
          QPixmap(QString(beginning_path_ + "/bd/photo_product/%1/logo.jpg")
                      .arg(line.data_.first())));
      layout_h->GetLabel()->setText(
          QString("<p align=left><font size=5>%1</font><br/>"
                  "<br/><font size=3>%2 руб. %3 шт.</font></p>")
              .arg(line.data_[1], line.data_[3], line.data_[7]));
      layout_h->GetSpinBox()->setObjectName(
          QString("spin_%1_%2").arg(line.data_.first(), line.data_[3]));
      layout_h->GetSpinBox()->setRange(0, line.data_[7].toInt());
      layout_h->GetSpinBox()->setValue(
          order_.QuantityInOrder(line.data_[0].toInt()).first);
      if (layout_h->GetSpinBox()->value())
        layout_h->GetSumma()->setText(QString("%1").arg(QString::number(
            layout_h->GetSpinBox()->value() * line.data_[3].toDouble())));

      connect(layout_h->GetLabel(), &MyLabel::clicked, this,
              &MainWindow::Label3Clicked);
      connect(layout_h->GetSpinBox(), &QSpinBox::valueChanged, &order_,
              &Order::AddProduct);
      connect(layout_h->GetSpinBox(), &QSpinBox::valueChanged, this,
              &MainWindow::PriceChange);
...
void MainWindow::ProductWindow(QScrollArea *scroll_area, QString id) {
  ...
  QPixmap p(QString(beginning_path_ + "/bd/photo_product/%1/1.jpg")
                .arg(data.data_[0]));
  ...
  ui->label_12->setText(
      QString("%2\n\nАртикул - %1")
          .arg(data.data_[5], data.data_[6].replace("<?>", "\n")));
...
void MainWindow::OrderWindow(QScrollArea *scroll_area, QString file_path) {
  ...
  if (file.open(QFile::ReadOnly)) {
    QString line = file.readLine();
    ui->label_17->setText(
        QString("%L1").arg(line.section(';', 2, 2).trimmed().toDouble()));
    while (!file.atEnd()) {
      line = file.readLine();
      ...
      auto data = model_.GetLine(id.toInt());
      layout_h->GetPhoto()->setPixmap(QPixmap(
          QString(beginning_path_ + "/bd/photo_product/%1/logo.jpg").arg(id)));
      layout_h->GetLabel()->setText(
          QString("<p align=left><font size=5>%1</font><br/>"
                  "<br/><font size=3>%2 руб. %3 шт.</font></p>")
              .arg(data.data_[1], data.data_[3], data.data_[7]));
      layout_h->GetSpinBox()->setRange(0, data.data_[7].toInt());
      layout_h->GetSpinBox()->setValue(count.toInt());
      if (layout_h->GetSpinBox()->value())
        layout_h->GetSumma()->setText(QString("%1").arg(QString::number(
            layout_h->GetSpinBox()->value() * data.data_[3].toDouble())));
      layout_h->GetSpinBox()->setReadOnly(true);
      connect(layout_h->GetLabel(), &MyLabel::clicked, this,
              &MainWindow::Label5Clicked);
...
void MainWindow::LabelClicked(QString text) {
  ...
  //  QFuture<void> future = QtConcurrent::run(api_, Api::RequestAddOrder);
  //    QtConcurrent::run(ContractOnWidgit, ui->scrollArea);
...
void MainWindow::on_pushButton_12_clicked() {
  if (ui->lineEdit_7->text().isEmpty() || ui->lineEdit_9->text().isEmpty() ||
      ui->lineEdit_10->text().isEmpty() || data_.empty())
    return;
  QStringList data;
...
void MainWindow::on_pushButton_19_clicked() {
  if (ui->lineEdit_13->text().isEmpty() || ui->lineEdit_8->text().isEmpty() ||
      ui->lineEdit_11->text().isEmpty() || data_.empty() ||
      ui->comboBox_2->isWindowModified() ||
  ...
  data << GetIdBD(QFile(beginning_path_ + "/bd/contract.csv"),
                  ui->comboBox_2->currentText());
  data << ui->lineEdit_11->text().trimmed();
  data << GetIdBD(QFile(beginning_path_ + "/bd/category.csv"),
...
void MainWindow::on_pushButton_14_clicked() {
  ...
  QString dir = picturesLocations.isEmpty() ? QDir::currentPath()
                                            : picturesLocations.last();
  data_.push_front(
      QFileDialog::getOpenFileName(this, "Select one file to open", dir,
                                   "Images (*.jpeg *.jpg *.JPG *.JPEG)"));
}
...
void MainWindow::on_pushButton_20_clicked() {
  ...
  QString dir = picturesLocations.isEmpty() ? QDir::currentPath()
                                            : picturesLocations.last();
  data_ = QFileDialog::getOpenFileNames(this, "Select one file to open", dir,
                                        "Images (*.jpeg *.jpg *.JPG *.JPEG)");
}
...
QList<QPair<QString, QString>> MainWindow::GetIdName(QFile file) {
  ...
  if (file.open(QIODevice::ReadOnly)) {
    while (!file.atEnd()) {
      QString line = file.readLine();
      result << QPair<QString, QString>(line.section(';', 0, 0),
...
void MainWindow::on_spinBox_valueChanged(int arg1) {
  ui->label_13->setText(
      QString::number(ui->label_9->text().section(' ', 0, 0).toDouble() *
                      arg1) +
      " руб.");
}
...
void MainWindow::on_pushButton_22_clicked() {
  ...
  directories << beginning_path_ + "/bd_local/orders"
              << "Локально"
              << "red"
              << "white"
              << "local_";
  QStringList separators;
  separators << beginning_path_ + "/bd/orders"
             << "В базе"
             << "green"
             << "white"
             << "bd_";
...
void MainWindow::ResponseAddOrder() {
  int status_code =
      api_.reply_->attribute(QNetworkRequest::HttpStatusCodeAttribute)
          .value<int>();
  if (status_code == 200) {
    QString file_name = QString(api_.reply_->rawHeader("Content-Disposition"))
                            .section('"', 1, 1);
