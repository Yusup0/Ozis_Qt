...
int Order::GetIdRetailer() {
    return id_retailer_;
}

void Order::SetIdRetailer(int id) {
    id_retailer_ = id;
}
...
void Order::PlaceOrder() {
    ...
    if (file.open(QIODevice::ReadWrite)) {
        ...
        out << id_retailer_ << ";" << QDateTime::currentDateTime().toString("HH:mm dd.MM.yy") << ";" << total_price_ <<"\n";
        while (it != orders_.end()) {
            out << QString("%1;%2;%3\n").arg(QString::number(it.key()), QString::number(it.value().first), QString::number(it.value().second));
            ++it;
        }
        file.close();
    }
...
void Order::AddProductFromFile(QString file_path) {
    ...
    if(file.open(QFile::ReadOnly)) {
        ...
        while (!file.atEnd()) {
            ...
            orders_.insert((*it).toInt(), QPair<int, double>((*(++it)).toInt(), (*(++it)).toDouble()));
        }
    }
}
