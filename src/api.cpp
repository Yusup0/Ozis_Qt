...
void Api::RequestBearer(User &user) {
  ...
  body = "grant_type=&username=" + body + "&password=" + user.GetPassword() +
...
void Api::RequestAddContract(User &user, QString contract) {
  QNetworkRequest request;
  request.setUrl(QUrl("https://ozisapp.ru/base/contract/add?contract_str=" +
                      QUrl::toPercentEncoding(contract)));
  request.setRawHeader("accept", "application/json");
  request.setRawHeader("Authorization",
                       ("Bearer " + user.GetBearer()).toUtf8());
  reply_ = manager_.post(request, QByteArray());
}

void Api::RequestAddCategory(User &user, QString category) {
  QNetworkRequest request;
  request.setUrl(QUrl("https://ozisapp.ru/base/category/add?category_str=" +
...
void Api::RequestAddRetailer(User &user, QStringList data) {
  ...
  textPart.setHeader(QNetworkRequest::ContentDispositionHeader,
                     QVariant("form-data; name=\"new_store\""));
  textPart.setBody(doc.toJson());
  QHttpPart imagePart;
  imagePart.setHeader(
      QNetworkRequest::ContentDispositionHeader,
      QVariant("form-data; name=\"photo\";filename=\"send.zip\""));
...
void Api::RequestAddProduct(User &user, QStringList data) {
  QStringList::Iterator it = data.begin();
  QNetworkRequest request(
      QUrl("https://ozisapp.ru/base/product/add?count1=" + *it++));
  request.setRawHeader("Authorization",
                       ("Bearer " + user.GetBearer()).toUtf8());
  ...
  QJsonDocument doc(obj);
  textPart.setHeader(QNetworkRequest::ContentDispositionHeader,
                     QVariant("form-data; name=\"new_product\""));
  textPart.setBody(doc.toJson());
  QHttpPart imagePart;
  imagePart.setHeader(
      QNetworkRequest::ContentDispositionHeader,
      QVariant("form-data; name=\"archive\";filename=\"send.zip\""));
  QFile *file = new QFile(*it);
  file->open(QIODevice::ReadOnly);
  imagePart.setBodyDevice(file);
  file->setParent(multiPart);
...
void Api::RequestAddOrder(User &user, QString path) {
  ...
  QString r = QString("form-data; name=\"order\";filename=\"%1\"")
                  .arg(path.split('/').back().trimmed());
  file_image_part.setHeader(QNetworkRequest::ContentDispositionHeader,
                            QVariant(r));
...
//     "application/x-www-form-urlencoded"}}); std::string login =
//     user.GetLogin(); login.replace(login.find("@"), 1, "%40");
//     session.SetBody(cpr::Body{"grant_type=&username=" + login + "&password="
//     + user.GetPassword() + "&scope=&client_id=&client_secret='"});
...
//     + url_encode(contract)}); session.SetHeader(cpr::Header{{"accept",
//     "application/json"}}); session.SetBearer(cpr::Bearer(user.GetBearer()));
//     cpr::Response res = session.Post();
//     return res.status_code;
//     return 0;
// }
...
//     + url_encode(category)}); session.SetHeader(cpr::Header{{"accept",
//     "application/json"}}); session.SetBearer(cpr::Bearer(user.GetBearer()));
//     cpr::Response res = session.Post();
//     return res.status_code;
//     return 0;
