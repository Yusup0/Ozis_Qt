...
void Zip(QStringList files_path, QString archive_path, bool save_name) {
    ...
    for (auto path: files_path) {
        QString file_name = save_name ? path.split('/').back() : QString::number(i++) + ".jpg";
        QFile file(path);
    ...
    writer->addFile(save_name ? "logo_" + files_path.first().split('/').back()  : "logo.jpg" ,
...
QByteArray GetLogo(QString path) {
    QPixmap pixmap(path);
    pixmap = pixmap.scaled(80, 80, Qt::IgnoreAspectRatio,  Qt::SmoothTransformation);
    QByteArray bArray;
