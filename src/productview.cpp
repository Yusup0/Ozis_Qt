...
//    photo_->setPixmap(QPixmap(QString(beginning_path_ + "/bd/photo_product/%1/logo.jpg").arg(data.data_.first())));
...
MyLabel *ProductView::GetLabel() {
    return label_;
}

MyLabel *ProductView::GetPhoto() {
    return photo_;
}

QSpinBox *ProductView::GetSpinBox() {
    return spin_box_;
}

QLabel *ProductView::GetSumma() {
    return summa_;
}
